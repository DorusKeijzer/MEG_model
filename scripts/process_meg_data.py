...
def process_meg_data(h5_filepath, downsample_factor=5, save_dir="./processed_data"):
    with h5py.File(h5_filepath, 'r') as f:
        # Assuming dataset inside h5 file
        dataset_name = list(f.keys())[0]
        data = f[dataset_name][()]  # shape (248, time_steps)
    
    # Transpose to shape (time_steps, 248) for easier processing
    data = data.T

    # Downsample temporally
    data_ds = data[::downsample_factor]

    # Initialize the spatially reshaped data array
    num_time_steps = data_ds.shape[0]
    spatial_data = np.zeros((num_time_steps, 20, 21), dtype=data.dtype)

    # Map channels to spatial grid
    for r in range(20):
        for c in range(21):
            ch_idx = layout_matrix[r, c]
            if ch_idx >= 0:
                spatial_data[:, r, c] = data_ds[:, ch_idx]

    # Save
    os.makedirs(save_dir, exist_ok=True)
    base_name = get_dataset_name(h5_filepath)
    save_path = os.path.join(save_dir, f"{base_name}_downsampled_spatial.npy")
